...
def insert_song(title, artist, playlist_id, spotipy_id, url):
    ...
    try:
        cursor.execute("SELECT SongID From Song Where spotipy_id = ?", (spotipy_id,))
        row = cursor.fetchone()
        if row is not None:
            pass
...
def insert_songs_sent(match_id, sender_id, reciever_id, song_id):
    '''Inserts a new record into the SongsSent table.'''
    conn = connect_to_database()
    cursor = conn.cursor()

    try:
        cursor.execute("INSERT INTO SongsSent (match_id, sender_id, receiver_id, song_id) VALUES (?, ?, ?, ?)", (match_id, sender_id, reciever_id, song_id))
        conn.commit()
    except sqlite3.Error as error:
        print(error)
    finally:
